-- Founding Member Badge for First 20 Users (FIXED VERSION)
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'badges') THEN
    RAISE EXCEPTION 'badges table does not exist. Please create the badges table first.';
  END IF;
  
  -- Check if user_badges table exists
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_badges') THEN
    RAISE EXCEPTION 'user_badges table does not exist. Please create the user_badges table first.';
  END IF;
...
DO $$
BEGIN
  -- Insert the badge if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM badges WHERE name = 'Founding Member') THEN
    ...
    RAISE NOTICE 'Founding Member badge created successfully';
  ELSE
    ...
    RAISE NOTICE 'Founding Member badge updated successfully';
  END IF;
END $$;
...
BEGIN
  ...
  IF user_position <= 20 THEN
    ...
    IF founding_badge_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'Founding Member badge awarded to user % (position %)', NEW.id, user_position;
    END IF;
  END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error in check_founding_member_badge: %', SQLERRM;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_founding_member_badge') THEN
    DROP TRIGGER trigger_founding_member_badge ON user_profiles;
...
-- 5. Create a function to manually award founding member badge (for admin use)
...
-- 6. Award the badge to existing users who are in the first 20 (ordered by created_at)
DO $$
DECLARE
  founding_badge_id UUID;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Founding Member badge not found, cannot award to existing users';
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error awarding badges to existing users: %', SQLERRM;
END $$;
...
DECLARE
  ...
  total_users INTEGER;
  badge_recipients INTEGER;
BEGIN
  -- Check if badge exists
  SELECT EXISTS(SELECT 1 FROM badges WHERE name = 'Founding Member') INTO badge_exists;
  
  -- Get counts
  SELECT COUNT(*) INTO total_users FROM user_profiles;
  
  SELECT COUNT(*) INTO badge_recipients 
  FROM user_badges ub
  JOIN badges b ON ub.badge_id = b.id 
  WHERE b.name = 'Founding Member';
  
  RAISE NOTICE '=== FOUNDING MEMBER BADGE STATUS ===';
  RAISE NOTICE 'Badge exists: %', badge_exists;
  RAISE NOTICE 'Total users: %', total_users;
  RAISE NOTICE 'Users with Founding Member badge: %', badge_recipients;
  RAISE NOTICE '=====================================';
END $$;

-- 8. Show the badge details if it exists
SELECT 
...
-- 9. Show current badge recipients with their position (if any exist)
SELECT 
